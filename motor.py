...
if __name__ == '__main__':
    ...
    with open("data_for_zzh", 'rb') as f_pos:
        data = load(f_pos)[1]
    ...
    for num in [0, 1, 2, 3, 5]:  #:
        ...
        for i in range(1, len(data)):
            time_list, spd_list, run_type = build_math_path(v, vmax_value[num], pos[i - 1], data[i][num + 1],
                                                            a_value[num], a_value[num])
            pos_step = get_pos_by_ts(data[i][0] - data[i - 1][0], time_list, spd_list)[0]
            pos.append(pos[i - 1] + pos_step)
            v = get_math_spd_by_ts(data[i][0] - data[i - 1][0], time_list, spd_list)[0]
            if abs(pos - data[i][num + 1]) > 1e-3:
                # print(time_list, spd_list)
                # print(i, ":", num, ":", data[i+1][0]-data[i][0], pos_step, data[i+1][num + 1]-data[i][num + 1])
                print(i, ":", num, ":", data[i][0] - data[i - 1][0], ":", pos[i] - data[i][num + 1])
